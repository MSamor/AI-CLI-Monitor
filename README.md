...
2. 点击「开启灵动岛」会打开一个贴近 mac 状态栏右上角的置顶小胶囊。
3. 点击灵动岛会展开详情，展示 Claude、Codex、蓝牙、灯控和最近事件；鼠标移出后自动收回。
4. 没有硬件时可以点击「模拟」切换到模拟蓝牙通道。
5. 「手动灯控」可以直接发送 `G/Y/R/B` 指令，用来验证 UI 和硬件链路。
6. 如果连接真实 Pico，蓝牙面板会显示扫描、连接、重连或错误状态。
